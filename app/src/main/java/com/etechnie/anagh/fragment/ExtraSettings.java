...
//            activity.currentFragment = activity.categories_4;
            ...
            if (activity.fragmentDoctor == null) {
                Bundle bundle = new Bundle();
                bundle.putBoolean("isHeaderVisible", false);
                activity.fragmentDoctor = new FragmentDoctor();
                activity.fragmentDoctor.setArguments(bundle);
                fragmentManager.beginTransaction()
                        .hide(activity.currentFragment)
                        .add(R.id.main_fragment, activity.fragmentDoctor)
                        .setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE)
                        .addToBackStack(null).commit();
            } else {
                if (fragmentManager.findFragmentByTag(getString(R.string.categoryStyle5)) == null)
                fragmentManager.beginTransaction().hide(activity.currentFragment).hide(activity.extraSettings).add(R.id.main_fragment, activity.fragmentDoctor).setTransition(FragmentTransaction.TRANSIT_FRAGMENT_OPEN).addToBackStack(null).commit();
                else
                    fragmentManager.beginTransaction().hide(activity.currentFragment).hide(activity.extraSettings).show(activity.fragmentDoctor).setTransition(FragmentTransaction.TRANSIT_FRAGMENT_OPEN).addToBackStack(null).commit();
            }
