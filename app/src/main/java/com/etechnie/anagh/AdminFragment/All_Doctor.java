...
import com.etechnie.anagh.AdminAdapter.DoctorListAdapter;
...
public class All_Doctor extends Fragment {
    ...
    DoctorListAdapter ordersListAdapter;

    List<DoctorModel> ordersList = new ArrayList<>();
    int pageNo = 0;
int LANGUAGE_ID=0;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        ordersListAdapter = new DoctorListAdapter(getContext(), customerID, ordersList, new DoctorListAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(DoctorModel item,int id) {
        ...
        RequestMyOrders(pageNo, sortBy,LANGUAGE_ID);
        add.setText("Create Doctor Slot");
    ...
    private void addOrders(List<DoctorModel> orderData) {
        ...
        ordersListAdapter = new DoctorListAdapter(getContext(), customerID, ordersList, new DoctorListAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(DoctorModel item,int state) {
                if(state==1){
                    Bundle bundle = new Bundle();
                    Gson gson = new Gson();
                    ...
                    bundle.putString("service_type", "Medical");
                    Fragment fragment;
                    FragmentManager fragmentManager = getActivity().getSupportFragmentManager();
                    // Navigate to Products Fragment
    ...
    public void RequestMyOrders(int pageNumber, String sortBy,int LANGUAGE_ID) {
            ...
            public void onResponse(Call<ResponseData<List<DoctorModel>>> call, retrofit2.Response<ResponseData<List<DoctorModel>>> response) {
                ...
                if (response.isSuccessful()) {
                    if (String.valueOf(response.body().getCode()).equalsIgnoreCase("1")) {
                        ...
                        addOrders(response.body().getData());
                    }
